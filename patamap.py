...
<body>
    ...
    <script>
        ...
        // 都道府県のメモテキストを辞書化する関数（複数口コミ・長文・改行混在対応の超堅牢パーサー）
        function parsePrefMemos(memoText) {{
            ...
            rawLines.forEach(line => {{
                ...
                const hasColon = trimmed.indexOf(':') !== -1 || trimmed.indexOf('：') !== -1;
                if (hasColon) {{
                    if (currentEntry) {{
            ...
            // 2. 1行内に読点「、」で複数スポットの口コミが連結されている場合を分割
            // （肯定先読み：読点「、」の直後に「コロンを含むスポット名」が続く境界で分割）
            const finalEntries = [];
            rawEntries.forEach(entry => {{
                const subParts = entry.split(/、(?=[^:：]+[:：])/);
                subParts.forEach(p => {{
                    if (p.trim()) finalEntries.push(p.trim());
                }});
            }});

            // 3. 各エントリから「スポット名」と「口コミ本文」を抽出して格納
            finalEntries.forEach(entry => {{
                ...
                if (sepIdx !== -1) {{
                    ...
                    const key = rawKey.replace(/<rt>[^<]*<\\/rt>/g, '').replace(/<[^>]+>/g, '').trim();
                            ...
                            <div class="tooltip-card">
                                ...
                                <div class="tooltip-section">
                                    <div class="tooltip-title sightseeing-title">
                                        <span>📸 観光地</span>
                                        <button type="button" class="add-item-btn add-sightseeing-btn" onclick="openAddModal('${{prefName}}', 'sightseeing', '')" title="【${{prefName}}】に新しい観光地を追加">➕ 追加</button>
                                    </div>
                                ...
                                <div class="tooltip-section">
                                    <div class="tooltip-title food-title">
                                        <span>🍽️ ご当地グルメ</span>
                                        <button type="button" class="add-item-btn add-food-btn" onclick="openAddModal('${{prefName}}', 'food', '')" title="【${{prefName}}】に新しいご当地グルメを追加">➕ 追加</button>
                                    </div>
                                ...
                                <div class="tooltip-section">
                                    <div class="tooltip-title drink-title">
                                        <span>🍶 地酒</span>
                                        <button type="button" class="add-item-btn add-drink-btn" onclick="openAddModal('${{prefName}}', 'drink', '')" title="【${{prefName}}】に新しい地酒を追加">➕ 追加</button>
                                    </div>
                                ...
                                <div class="tooltip-section">
                                    <div class="tooltip-title souvenir-title">
                                        <span>🎁 お土産</span>
                                        <button type="button" class="add-item-btn add-souvenir-btn" onclick="openAddModal('${{prefName}}', 'souvenir', '')" title="【${{prefName}}】に新しいお土産を追加">➕ 追加</button>
                                    </div>
