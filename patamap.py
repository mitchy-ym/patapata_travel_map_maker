...
<body>
    ...
    <header class="header-bar">
        ...
        <div class="header-actions">
            <button type="button" class="btn btn-primary" onclick="handleDirectSave()" style="background:#15803d; border-color:#166534;" title="編集したデータをGitHubにコミットして本番マップを自動更新">💾 本番マップに保存・更新</button>
            <button type="button" class="btn btn-secondary" onclick="openGithubModal()" title="GitHub Personal Access Token (PAT) を設定・確認">⚙️ トークン設定</button>
            <button type="button" class="btn btn-danger" onclick="resetAllChanges()" title="未保存の編集を破棄して初期状態に戻す">🔄 リセット</button>
            <a href="index.html" target="_blank" class="btn btn-secondary" title="公開マップを別タブで開く">🌐 公開マップ</a>
        </div>
    </header>
    ...
    <script>
        ...
        function loadSavedGitHubSettings() {{
            ...
            const repo = localStorage.getItem('patamap_gh_repo') || 'mitchy-ym/patapata_travel_map_maker';
            const branch = localStorage.getItem('patamap_gh_branch') || 'main';
        ...
        function handleDirectSave() {{
            const token = localStorage.getItem('patamap_gh_token');
            if (!token) {{
                alert('💡 初回設定: GitHub Personal Access Token (PAT) を設定してください。\\n（※ブラウザに記憶されるため、次回から1クリックで保存できるようになります）');
                openGithubModal();
                return;
            }}

            if (confirm('編集したデータを GitHub に保存して、本番マップを更新しますか？\\n（※約15秒後に自動で本番サイトが更新されます）')) {{
                const repo = localStorage.getItem('patamap_gh_repo') || 'mitchy-ym/patapata_travel_map_maker';
                const branch = localStorage.getItem('patamap_gh_branch') || 'main';
                executeGitHubCommit(token, repo, branch, 'Update patamap_data.json via admin editor');
            }}
        }}
        ...
        async function commitToGitHub(e) {{
            ...
            await executeGitHubCommit(token, repo, branch, message);
        }}

        async function executeGitHubCommit(token, repo, branch, message) {{
            showToast('⏳ GitHub にデータを送信中...');
            ...
            try {{
                ...
                if (!getRes.ok) {{
                    if (getRes.status === 401) {{
                        throw new Error('トークンが無効または期限切れです。トークン設定から再設定してください。');
                    }}
